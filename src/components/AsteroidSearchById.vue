...
<div> 
  ...
  <input class="asteroid_id_bar" type="text" v-model="asteroid_id" ><br><br>
  <input type="submit" value="Submit" >
  </div>
  </form>   
    <table id="asteroid_table_by_id">  <tr>
    <th>Asteroid Name</th>
    <th>Asteroid Id</th> 
    <th>NASA JPN URL</th>   
  </tr>
  <tr v-for="asteroidDataById in asteroidDataByIdList" :key="asteroidDataById.id">
  ...
  methods: {
    async getAsteroidDataById (form_event) {
        ...
        try {         
         ...
         `https://api.nasa.gov/neo/rest/v1/neo/${this.asteroid_id}?api_key=${apiKey}`
         );
          const json_data =  await response.json(); 
          
          finalAsteroidByIdData.push({"name":json_data.name, "id":json_data.id,"nasa_jpl_url":json_data.nasa_jpl_url});
